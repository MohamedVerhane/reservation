<x-frontend.dashboard-layout :title="__('auth.cd_history')">

    {{-- Page Header --}}
    <div class="mb-8 animate-fade-in-up" data-animate>
        <h1 class="text-2xl sm:text-3xl font-extrabold text-slate-900 dark:text-white">{{ __('auth.cd_history') }}</h1>
        <p class="text-slate-500 dark:text-slate-400 mt-1">{{ __('auth.cd_no_history_text') }}</p>
    </div>
    ...
    @if($reservations->count())
        <div class="space-y-5">
            @foreach($reservations as $reservation)
                ...
                @endphp
                <div class="bg-white/90 dark:bg-slate-800/90 backdrop-blur-xl rounded-2xl border border-slate-200 dark:border-slate-700 shadow-xl overflow-hidden transition-all duration-300 hover:shadow-2xl animate-fade-in-up" data-animate style="animation-delay: {{ ($loop->index * 80) + 100 }}ms">
                    {{-- Status Bar --}}
                    <div class="{{ $reservation->status === 'cancelled' ? 'bg-red-500' : 'bg-slate-400' }} px-6 py-2">
                        ...
                        <div class="flex flex-col lg:flex-row lg:items-center justify-between gap-6">
                            ...
                            <div class="flex items-center gap-4">
                                <div class="flex items-center gap-2">
                                    ...
                                    <span class="inline-flex items-center gap-1.5 rounded-full {{ $paymentBadge['bg'] }} px-3 py-1 text-xs font-semibold {{ $paymentBadge['text'] }}">
    ...
    @else
        <div class="bg-white/90 dark:bg-slate-800/90 backdrop-blur-xl rounded-2xl border border-slate-200 dark:border-slate-700 shadow-xl p-12 text-center animate-fade-in-up" data-animate>
            <i class="bi bi-clock-history text-6xl text-slate-300 dark:text-slate-600 mb-4 block"></i>
            <p class="text-slate-500 dark:text-slate-400 text-lg mb-2">{{ __('auth.cd_no_history') }}</p>
            <p class="text-slate-400 dark:text-slate-500 text-sm mb-6">{{ __('auth.cd_no_history_text') }}</p>
