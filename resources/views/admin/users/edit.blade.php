<x-layouts.admin :title="__('admin.title.edit', ['resource' => __('admin.nav.users')])" active="users">
    <div class="space-y-6">
        <nav class="flex items-center gap-2 text-sm text-slate-500 dark:text-slate-400">
            ...
            <a href="{{ route('admin.users.show', $user) }}" class="hover:text-indigo-600 dark:hover:text-indigo-400">{{ $user->name }}</a>
            <i class="bi bi-chevron-right text-xs"></i>
        ...
        <form method="POST" action="{{ route('admin.users.update', $user) }}">
            @csrf
            @method('PUT')
            <x-admin.user-form :user="$user" :submitLabel="__('admin.users.update')" />
        </form>
    </div>
</x-layouts.admin>
