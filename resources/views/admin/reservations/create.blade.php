<x-layouts.admin :title="__('admin.reservations.create_title')" active="reservations">
    ...
    <div class="mb-6">
        <div class="flex items-center gap-2 text-sm text-slate-400 dark:text-slate-500">
            ...
            <span class="text-slate-700 dark:text-slate-300 font-semibold">{{ __('admin.reservations.create_title') }}</span>
        </div>
    </div>
    ...
    <div x-data="reservationForm()" x-init="init()">
        ...
        <form method="POST" action="{{ route('admin.reservations.store') }}" data-ajax>
            @csrf
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                    ...
                    <div class="rounded-2xl border border-slate-200/80 dark:border-slate-800/80 bg-purple-50/60 dark:bg-slate-900 p-6">
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            ...
                            <div>
                                ...
                                <input type="number" name="guests" id="guests" min="1" max="20" value="{{ old('guests', 1) }}" required
                                    class="w-full rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 px-4 py-2.5 text-sm text-slate-900 dark:text-slate-100 focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500/20 outline-none transition-all" />
                                @error('guests')
                                    <p class="mt-1 text-xs text-red-500">{{ $message }}</p>
                                @enderror
                            ...
                            <div>
                                ...
                                <input type="number" name="children_count" id="children_count" min="0" max="10" value="{{ old('children_count', 0) }}"
                                    class="w-full rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 px-4 py-2.5 text-sm text-slate-900 dark:text-slate-100 focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500/20 outline-none transition-all" />
                            ...
                            <div class="sm:col-span-2">
                                ...
                                <textarea name="notes" id="notes" rows="3" maxlength="1000"
                                    ...
                                    placeholder="{{ __('admin.reservations.notes_placeholder') }}">{{ old('notes') }}</textarea>
                                @error('notes')
                                    <p class="mt-1 text-xs text-red-500">{{ $message }}</p>
                                @enderror
                            </div>
                ...
                <div class="space-y-6">
                    ...
                    {{-- Price Summary --}}
                    <div class="rounded-2xl border border-slate-200/80 dark:border-slate-800/80 bg-amber-50/60 dark:bg-slate-900 p-6 sticky top-6">
                        <h3 class="text-sm font-bold text-slate-800 dark:text-slate-200 mb-4">
                        ...
                        <button type="submit"
                            ...
                            :disabled="submitting">
                            <i class="bi bi-check-lg text-base"></i> {{ __('admin.reservations.create_submit') }}
                        </button>
