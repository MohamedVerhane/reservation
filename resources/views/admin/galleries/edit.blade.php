<x-layouts.admin :title="__('admin.title.edit', ['resource' => $gallery->title])" active="galleries">
    ...
    <div class="mb-6">
        <div class="flex items-center gap-2 text-sm text-slate-400 dark:text-slate-500">
            ...
            <a href="{{ route('admin.galleries.show', $gallery) }}" class="hover:text-indigo-600 dark:hover:text-indigo-400 transition-colors">{{ $gallery->title }}</a>
            <i class="bi bi-chevron-right text-xs"></i>
    ...
    <form method="POST" action="{{ route('admin.galleries.update', $gallery) }}" enctype="multipart/form-data">
        @csrf
        @method('PUT')
        <x-admin.gallery-form :gallery="$gallery" :hotels="$hotels" :submitLabel="__('admin.galleries.update')" />
    </form>
